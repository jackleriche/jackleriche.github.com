...
module com.camelot.iwg {
	...
	export class BallSelection extends Scene {
        ...
        private _ballsObject: GameObject      = null;
		...
		private _initBallSelection() {
			...
			var selectedBalls: GameObject 	= new GAMEOBJECT('selectedBalls', {w: 500, h: 100}, 2, this),
				balls: ClickableGameObject	= new CLICKABLEGAMEOBJECT('balls', {w: 800, h: 345}, 2, this ),
                choose: GameObject          = new GAMEOBJECT('choose', { w: 300, h: 50 }, 2, this);
		 ...
		 *	returns:		
		 */
		private _ballSelected(ball, auto?) {
			...
			if (!ball.selected) {
                ...
                TweenMax.delayedCall(1, () => {
                    if ( GLOBAL.getInstance().getFromGlobal('selectionMethod') === "choose"){
                        this._ballsObject.setEnabled(true);    
                    }  
                });
        ...
        private _updateMessage(num:number) {
            var	choose          = this.getChildByName('choose'),
                chooseBitmap 	= this.getChildByName('choose').getBitmap('choose6');
		 ...
		 *  returns:        null
		 */
		private _maxSelected(ballClick) {
			...
			timeline.to(choose6numbers.getCanvas(), 0.2, { alpha: 0})
                    .to( this._button.getCanvas(), 0.2, { alpha: 1, onComplete: () => {
                        // button set enabled
