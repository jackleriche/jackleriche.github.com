(function (window) {
	...
	R.cornerRevealTimeLine    	= new TimelineLite({delay: 0.2});
	R.promptAnimation			= new TimelineMax({repeat: -1});
	...
	R.ParseCheck_Ticket = function () {
        ...
        if (ticketCalcTotal != R._ticketPrizeAmount) {
       	 	core.IWG.ame('error', {mes: ['error 1 - no match prize total']});
        }
        ...
        if(cornerCount === 4) {
            ...
            if (R._ticketPrizeTier == 9) {
            	core.IWG.ame('error', {mes: ['not 4 corners']});
            }
        }
	}
	R.CURSORPOINTER = function(evt){
        ...
        var tarTimeline = new TimelineLite();
            tarTimeline.to(tar,0.2,{scaleX: 0.9, scaleY: 0.9})
                       .call(function(){tar.gotoAndStop(tar.hover)})
	...
	R.CURSORDEFAULT = function(evt){
		var tar 	= evt.target;
        var tarTimeline = new TimelineLite();
            tarTimeline.to(tar,0.2,{scaleX: 1, scaleY: 1})
                       .call(function(){
