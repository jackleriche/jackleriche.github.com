(function (window) {
    ...
    Swipe.prototype.enabled = function (bool, ele, mainContainer, buffer) {
        ...
        if (this.getEnabled()){
            ...
            hammertime.on("pan", function(ev){
                ...
                if(R.LOCK === false){
                    self.setDragDirection(ev.direction);
                    self.setDragDistance(ev.deltaX);
                    self.setStartX(ev.center.x);
                    var move = 0;
                    ...
                    if (move < -self.getLowerBound() && move > (-self.getUpperBound() - 360) ){
                        ...
                        } else if(self.getDragDirection() === 2) {
                            if(self.getDragDistance() < -self.getBuffer()){
                                R.MAINGAMECONTAINER.x = move + self.getBuffer();
                                if(self.getIsMoving() === false){
            ...
            hammertime.on("panend", function(e){
                ...
                if (s.x > 0){
                    ...
                    if ((movedBy < 0) && (movedBy < -(higher/10)) && (self.getDragDirection() === 2) ) {
                        self.moveRight();
                    } else if ( (movedBy > 0) && (movedBy > (lower/10)) && (self.getDragDirection() === 4) ) {
                        self.moveLeft();
                    } else {
    ...
    */
    ...
    Swipe.prototype.returnToEnd = function(){
        ...
        this.setCurrentStageX(-R.THIRDGAMEWINDOW.x);
    ...
    Swipe.prototype.updateToggle = function(num){
        ...
        toggles[this.getCurrentScreen()].alpha = 1;   
