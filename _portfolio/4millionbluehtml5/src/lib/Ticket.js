(function (window) {
    ...
    function sortGames() {
        ...
        checkValues( games[0], 't', 0, 7 );
        checkValues( games[0], 'p', 0, 10 );
        checkValues( games[1], "b0", 1, 9 );
        checkValues( games[1], "b1", 1, 9 );
        checkValues( games[1], "t", 3, 17 );
        checkValues( games[1], 'p', 0, 10 );
        checkValues( games[2], "y", 12, 30 );
        checkValues( games[2], "t", 10, 28 );
        checkValues( games[2], 'p', 0, 10 );
        checkValues( games[3], "v0", 1, 10 );
        checkValues( games[3], "v1", 1, 10 );
        checkValues( games[3], 'p', 0, 10 );
        checkValues( games[4], "s", 1, 13 );
        checkValues( games[4], 'p', 0, 10 );
    ...
    function checkValues(gameData, value, min, max) {
        ...
        for(var i = 0; i < gameData.go.length; i++ ){
            
            var turnData = gameData.go[i];
            if (turnData.hasOwnProperty(value) ) {
                var v = Number(turnData[value]);
                if(v !== isNaN){
                    if ( v < min || v > max ){
                        console.log(v);
                        core.IWG.ame('error', { mess: ['ticket value out of range']});
                    }   
                } else {
                    core.IWG.ame('error', { mess: ['ticket value is not a number']});
                }
    ...
    function checkEqualsTen(self){
        ...
        for (var i = 0; i < game2.go.length; i++) {
            ...
            if ( num1 + num2 !== t ){
                core.IWG.ame('error', { mess: ['error on equation']});
            }
    ...
    function checkMatchTwo(self){
        ...
        for (var i = 0; i < game4.go.length; i++){
            ...
            if( w === 1){
                if  (d0 !== d1 ){
                    core.IWG.ame('error', { mess: ['error on match2']});
                }
                // if a winner then add to bank for final check on ticket later
                var prize = parseInt(self.getPrizeList()[p]);
                self.setInlineBankCheck(prize);                
               ...
               if  (d0 === d1){
                    core.IWG.ame('error', { mess: ['error on win match2']});
                } 
            }
