(function (window) {
    ...
    Helper.showPrompt = function (screen) {
        ...
        if (isPrompting === false) {
            ...
            var gameWindows     = [ R.FIRSTGAMEWINDOW, R.SECONDGAMEWINDOW, R.THIRDGAMEWINDOW ],
                currentScreen   = R.SWIPE.getCurrentScreen();
            
            for ( var i = 0; i < gameWindows.length; i++ ){
                
                var game = gameWindows[i];
                                
                if ( game.getChildByName("arrow_right")) {
                    game.getChildByName("arrow_right").visible = false;
                }
                
                if ( game.getChildByName("arrow_left")) {
                    game.getChildByName("arrow_left").visible = false;
                }
                               
            }
            
            window.com.camelot.iwg.lib.Swipe.instance.updateToggle(currentScreen);
            R.FIRSTGAMEWINDOW.getChildByName('arrow_right').visible     = true;
            R.SECONDGAMEWINDOW.getChildByName('arrow_left').visible     = true;
            R.SECONDGAMEWINDOW.getChildByName('arrow_right').visible    = true;
            R.THIRDGAMEWINDOW.getChildByName('arrow_left').visible      = true;
            // now seperate for each screen to eliminate chance of arrows being shown 
            // from other screens
            // if(currentScreen === 0){
            //     R.FIRSTGAMEWINDOW.getChildByName('arrow_right').visible = true;
            //     if(R.GAMES[2].getIsFinished() != true || R.GAMES[3].getIsFinished() != true || R.GAMES[4].getIsFinished() != true){
            //         R.PROMPTS[0].restart(true);
            //     }
            // } else if (currentScreen === 1){
            //         R.SECONDGAMEWINDOW.getChildByName('arrow_left').visible = true;
            //         R.SECONDGAMEWINDOW.getChildByName('arrow_right').visible = true;
            //     if(R.GAMES[0].getIsFinished() != true || R.GAMES[1].getIsFinished() != true){
            //         R.PROMPTS[1].restart(true);            
            //     }
                
            //     if(R.GAMES[4].getIsFinished() != true ){
            //         R.PROMPTS[2].restart(true);                   
            //     }
                
            // } else if (currentScreen === 2){
            //     R.THIRDGAMEWINDOW.getChildByName('arrow_left').visible = true;
            //     if(R.GAMES[0].getIsFinished() != true || R.GAMES[1].getIsFinished() != true || R.GAMES[2].getIsFinished() != true || R.GAMES[3].getIsFinished() != true){   
            //         R.PROMPTS[3].restart(true);                 
            //     }
            // }
