 ...
 **/(function (e) {
    ...
    a._sendProgress = function (a) {
        var g;
        if (a instanceof Number)this.progress = a, g = {loaded:this.progress,
            total:1}; else if (g = a, this.progress = a.loaded / a.total, isNaN(this.progress) || this.progress == Infinity)this.progress = 0;
        g.target = this;
        if (this.onProgress)this.onProgress(g)
    };
...
(function (e) {
    var b = function (a) {
        this.initialize(a)
    }, a = b.prototype = new AbstractLoader;
    b.IMAGE = "image";
    b.SVG = "svg";
    b.SOUND = "sound";
    b.JSON = "json";
    ...
    a.initialize = function (a) {
        ...
        this.useXHR = a != false && e.XMLHttpRequest != null;
        this.determineCapabilities()
    };
    a.determineCapabilities =
        function () {
            var a = b.lib.BrowserDetect;
            if (a != null)b.TAG_LOAD_OGGS = a.isFirefox || a.isOpera
        };
    b.isBinary = function (a) {
        switch (a) {
            case b.IMAGE:
            case b.SOUND:
                return true;
    ...
    a.installPlugin = function (a) {
        if (!(a == null || a.getPreloadHandlers == null)) {
            ...
            if (a.types != null)for (var d = 0, b = a.types.length; d < b; d++)this.typeHandlers[a.types[d]] = a.callback;
            if (a.extensions != null)for (d = 0, b = a.extensions.length; d < b; d++)this.extensionHandlers[a.extensions[d]] = a.callback
        }
    };
    a.setMaxConnections =
        function (a) {
    ...
    a.loadFile = function (a, d) {
        a == null ? this._sendError({text:"File is null."}) : (this._addItem(a), d !== false && this.setPaused(false))
    };
    a.loadManifest = function (a, d) {
        var b;
        ...
        for (var c = 0, e = b.length; c < e; c++)this._addItem(b[c], false);
        d !== false && this._loadNext()
    };
    a.load = function () {
        this.setPaused(false)
    };
    a.getResult = function (a) {
        return this._loadedItemsById[a] || this._loadedItemsBySrc[a]
    };
    ...
    a._addItem = function (a) {
        ...
        a != null && (this._loadQueue.push(a), this._numItems++, this._updateProgress(), a.getItem().type == b.JAVASCRIPT && (this._scriptOrder.push(a.getItem()), this._loadedScripts.push(null)))
    };
    ...
    a._loadItem = function (a) {
        a.onProgress = b.proxy(this._handleProgress, this);
        a.onComplete = b.proxy(this._handleFileComplete,
            this);
        a.onError = b.proxy(this._handleFileError, this);
        this._currentLoads.push(a);
        a.load()
    };
    a._handleFileError = function (a) {
        var a = a.target, d = this._createResultData(a.getItem());
        this._numItemsLoaded++;
        this._updateProgress();
        this._sendError(d);
        this.stopOnError || (this._removeLoadItem(a), this._loadNext())
    };
    a._createResultData = function (a) {
        var d = {id:a.id, result:null, data:a.data, type:a.type, src:a.src};
        this._loadedItemsById[a.id] = d;
        return this._loadedItemsBySrc[a.src] = d
    };
    a._handleFileComplete = function (a) {
        var a =
            a.target, d = a.getItem(), c = this._createResultData(d);
        this._removeLoadItem(a);
        c.result = a instanceof b.lib.XHRLoader ? this._createResult(d, a.getResult()) : d.tag;
        switch (d.type) {
            case b.IMAGE:
                if (a instanceof b.lib.XHRLoader) {
                    var f = this;
                    c.result.onload = function () {
                        f._handleFileTagComplete(d, c)
                    };
                    return
                }
                break;
        ...
        this._handleFileTagComplete(d, c)
    };
    a._checkScriptLoadOrder = function () {
        for (var a =
            this._loadedScripts.length, d = 0; d < a; d++) {
            var b = this._loadedScripts[d];
            if (b === null)break;
            if (b !== true) {
                var c = this.getResult(b.src), b = this.getResult(b.id);
                b.result = c.result;
                this._handleFileTagComplete(c, b);
    ...
    a._handleFileTagComplete = function (a, b) {
        this._numItemsLoaded++;
        a.completeHandler && a.completeHandler(b);
        this._updateProgress();
        this._sendFileComplete(b);
        this._loadNext()
    };
    a._removeLoadItem = function (a) {
        for (var b = this._currentLoads.length, c = 0; c < b; c++)if (this._currentLoads[c] ==
            a) {
            this._currentLoads.splice(c, 1);
            break
        }
    };
    a._createResult = function (a, d) {
        var c = null, f;
        switch (a.type) {
            case b.IMAGE:
                c = this._createImage();
                break;
            case b.SOUND:
                c = a.tag || this._createAudio();
                break;
            case b.CSS:
                c = this._createLink();
                break;
            case b.JAVASCRIPT:
                c = this._createScript();
                break;
            case b.SVG:
                var c = this._createSVG(), e = this._createXML(d, "image/svg+xml");
                c.appendChild(e);
                break;
            case b.XML:
                f = this._createXML(d, "text/xml");
                break;
            case b.JSON:
            case b.TEXT:
                f = d
        ...
        if (c) {
            if (a.type == b.CSS)c.href = a.src; else if (a.type != b.SVG)c.src =
                a.src;
            return c
        } else return f
    };
    a._createXML = function (a, b) {
        var c;
        e.DOMParser ? (c = new DOMParser, c = c.parseFromString(a, b)) : (c = new ActiveXObject("Microsoft.XMLDOM"), c.async = false, c.loadXML(a));
        return c
    };
    a._handleProgress = function (a) {
        var a = a.target, b = this._createResultData(a.getItem());
        b.progress = a.progress;
        this._sendFileProgress(b);
        this._updateProgress()
    };
    a._updateProgress = function () {
        var a = this._numItemsLoaded / this._numItems, b = this._numItems - this._numItemsLoaded;
        if (b > 0) {
            for (var c = 0, e = 0, h = this._currentLoads.length; e <
                h; e++)c += this._currentLoads[e].progress;
            a += c / b * (b / this._numItems)
        }
        this._sendProgress({loaded:a, total:1})
    };
    a._createLoadItem = function (a) {
        ...
        switch (typeof a) {
            case "string":
                ...
                break;
            case "object":
                a instanceof HTMLAudioElement ? (c.tag = a, c.src = c.tag.src, c.type = b.SOUND) : c = a
        }
        c.ext = this._getNameAfter(c.src, ".");
        if (!c.type)c.type = this.getType(c.ext);
        if (c.id == null || c.id == "")c.id = c.src;
        if (a = this.typeHandlers[c.type] || this.extensionHandlers[c.ext]) {
            a = a(c.src, c.type, c.id, c.data);
            if (a === false)return null;
            else if (a !== true) {
                if (a.src != null)c.src = a.src;
                if (a.id != null)c.id = a.id;
                if (a.tag != null && a.tag.load instanceof Function)c.tag = a.tag
            }
            c.ext = this._getNameAfter(c.src, ".")
        }
        a = this.useXHR;
        switch (c.type) {
            ...
            case b.TEXT:
                ...
                break;
            case b.SOUND:
                c.ext == "ogg" && b.TAG_LOAD_OGGS && (a = false)
        }
        return this.useXHR == true && (c.type == b.IMAGE || c.type == b.SVG) ? (c = this._createTagItem(c), c.useXHR = true, c) : a ? new b.lib.XHRLoader(c) : c.tag ? new b.lib.TagLoader(c) : this._createTagItem(c)
    };
    a._createTagItem = function (a) {
        ...
        switch (a.type) {
            ...
            case b.JAVASCRIPT:
                ...
                break;
            case b.SVG:
                e = "data", c = this._createSVG()
        }
        a.tag = c;
        return new b.lib.TagLoader(a, e, f)
    };
    ...
    a._getNameAfter = function (a, c) {
        var b = a.lastIndexOf(c), b = a.substr(b + 1), e = b.lastIndexOf(/[\b|\?|\#|\s]/);
        return e == -1 ? b : b.substr(0, e)
    };
    ...
    c.init = function () {
        ...
        c.isOpera = e.opera != null;

    };
    c.init();
    b.lib.BrowserDetect = c
})(window);
(function () {
    ...
    b.init = function (a, c, b) {
        ...
        this._srcAttr = c || "src";
        this.useXHR = b == true;
        this.isAudio = a.tag instanceof HTMLAudioElement;
    ...
    b.loadXHR = function () {
        var a =
            this.getItem(), a = new PreloadJS.lib.XHRLoader(a);
        a.onProgress = PreloadJS.proxy(this._handleProgress, this);
        a.onFileLoad = PreloadJS.proxy(this._handleXHRComplete, this);
        a.onComplete = PreloadJS.proxy(this._handleXHRComplete, this);
        a.onError = PreloadJS.proxy(this._handleLoadError, this);
        a.load()
    };
    b._handleXHRComplete = function (a) {
        this._clean();
        a.target.onFileLoad = null;
        a.target.onComplete = null;
        var c = a.target.getItem();
        a.target.getResult();
        c.type == PreloadJS.IMAGE ? (c.tag.onload = PreloadJS.proxy(this._sendComplete,
            this), c.tag.src = c.src) : (c.tag[this._srcAttr] = c.src, this._sendComplete())
    };
    b._handleLoadError = function (a) {
        a.error && a.error.code == 101 ? this.loadTag() : (this._clean(), this._sendError(a))
    };
    b.loadTag = function () {
        var a = this.getItem(), c = a.tag;
        ...
        if (this.isAudio)c.src = null, c.preload = "auto", c.setAttribute("data-temp", "true");
        c.onerror = PreloadJS.proxy(this._handleLoadError,
            this);
        c.onprogress = PreloadJS.proxy(this._handleProgress, this);
        this.isAudio ? (c.onstalled = PreloadJS.proxy(this._handleStalled, this), c.addEventListener("canplaythrough", this.tagCompleteProxy, false)) : c.onload = PreloadJS.proxy(this._handleTagLoad, this);
        c[this._srcAttr] = a.src;
        a.type == PreloadJS.SVG && document.getElementsByTagName("body")[0].appendChild(c);
        a = a.type == PreloadJS.SOUND && a.ext == "ogg" && PreloadJS.lib.BrowserDetect.isFirefox;
        c.load != null && !a && c.load()
    };
    ...
    b._handleTagLoad = function () {
        ...
        clearTimeout(this._loadTimeOutTimeout);
        if (!(this.loaded || this.isAudio && a.readyState !== 4))this.getItem().type == PreloadJS.SVG && document.getElementsByTagName("body")[0].removeChild(a), this.loaded = true, this._clean(), this._sendComplete()
    };
    b._clean = function () {
        clearTimeout(this._loadTimeOutTimeout);
        var a = this.getItem().tag;
...
(function (e) {
    ...
    a._createXHR =
        function (a) {
            this._xhrLevel = 1;
            if (e.ArrayBuffer)this._xhrLevel = 2;
            if (e.XMLHttpRequest)this._request = new XMLHttpRequest; else try {
                this._request = new ActiveXObject("MSXML2.XMLHTTP.3.0")
            } catch (b) {
                return null
            }
            a.type == PreloadJS.TEXT && this._request.overrideMimeType && this._request.overrideMimeType("text/plain; charset=x-user-defined");
            this._request.open("GET", a.src, true);
            if (PreloadJS.isBinary(a.type))this._request.responseType = "arraybuffer";
            return true
        };
    a._clean = function () {
        clearTimeout(this._loadTimeOutTimeout);
